...
public:
	...
	//////////////////////////////////////////////////////////////////////////
	//Async trace to find and call the enemy guards nearby.
	//Perform the trace request.
	FTraceHandle RequestTrace(); //actually do the trace request.
	//Delegate function for the Async trace.
	void OnTraceCompleted(const FTraceHandle& Handle, FTraceDatum& Data);
	void GuardsHitResults(const FTraceDatum& TraceData);

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
		TEnumAsByte<ETraceTypeQuery> EnemyTraceType;

	FTraceHandle LastTraceHandle;
	//The delegate that is fired when the trace is completed.
